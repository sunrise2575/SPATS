...
def year_of_total(df: CombinedDatetimelikeProperties) -> np.ndarray:
    return (df.year - df.year.min()) / (df.year.max() - df.year.min() + 1)
...
def minute_of_day(df: CombinedDatetimelikeProperties) -> np.ndarray:
    return (df.hour * 60 + df.minute) / (24 * 60)
...
def embed_time(tsf: TSF, query: str = None):
    ...
    assert query in TEMPORAL_EMBED_FUNC.keys(), f'Error: query {query} is not supported'
